...
const boundary = new BoundaryShell({ operatorId: 'operator' });
const firstContact = new FirstContactProtocol({ operatorLabel: 'operator' });
...
rl.on('line', async (line) => {
  ...
  if (input === 'eye:on') { vision.open('camera'); rl.prompt(); return; }
  if (input === 'eye:off') { vision.close(); rl.prompt(); return; }
  if (input === 'eye:state') { console.log(JSON.stringify(vision.state(), null, 2)); rl.prompt(); return; }

  if (input === 'presence:on') { console.log(JSON.stringify(presence.open('local'), null, 2)); rl.prompt(); return; }
  if (input === 'presence:off') { console.log(JSON.stringify(presence.close('closed'), null, 2)); rl.prompt(); return; }
  if (input === 'presence:state') { console.log(JSON.stringify(presence.state(), null, 2)); rl.prompt(); return; }
  ...
  if (input.startsWith('presence:sense')) {
    ...
    const residue = presence.sense({
      ...
      rhythm: 'nearby',
      tone: 'unspecified',
      sourceHint: 'manual-marker',
      meaning: note || 'Presence marker supplied without interpretation requirement.'
    });
  ...
  if (presence.state().opened) context.push(`[presence:open sessionId=${presence.state().sessionId}]`);
