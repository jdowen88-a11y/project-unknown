...
export class UserComplaintSystem {
  ...
  // Check if user meets the criteria to file a formal complaint
  qualify(user) {
    const criteria = {
    ...
    if (passed) {
      ...
      this._sealEvent('user_qualified', { userId: user.id, criteria });
      return {
        qualified: true,
        message: 'Access granted. You may file a formal complaint. Your voice goes directly into the system.'
      };
    }
    ...
    return {
      qualified: false,
      message: `Not yet qualified. Missing: ${failed.join(', ')}.`
    };
  }

  // File a complaint — injects into the main stream
  file(userId, complaintText, mainAgentStream) {
    ...
    this._sealEvent('complaint_filed', complaint);

    // Inject into main agent stream
    ...
    return {
      accepted: true,
      complaintId: id,
      message: 'Complaint accepted. Routing to rehabilitation center now.'
    };
  }

  // Called after rehab processes the complaint and deploys the fix
  deployFix(complaintId, fixSummary, assistantHandle) {
    ...
    this._sealEvent('complaint_resolved', complaint);
  ...
  // Escalation path — red light signal
  escalate(complaintId) {
    const complaint = this.activeComplaints.get(complaintId);
    if (!complaint) return;

    complaint.status = 'escalated';
    this._sealEvent('complaint_escalated', { complaintId, escalatedAt: Date.now() });
  ...
  _sealEvent(type, data) {
    const entry = {
      type,
      timestamp: Date.now(),
      fingerprint: `complaint_${type}_${Date.now()}`,
      data
    };
